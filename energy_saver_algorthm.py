...
def find_capture_voltage(duration_val: int or float,
                         duration_data, voltage_amp_data):
    """
    ...
    Returns:
        capture_voltage (int or float): voltage amplitude to caputre the
                                        myocardial tissue
    """
    ...
    print("The nearest duration value to the one you entered \
is: {} ms".format(round(nearest_duration, 3)))
    print("The capture voltage is {} V at a duration \
of {} ms".format(capture_voltage, round(nearest_duration, 3)))
    return capture_voltage


def find_rheobase_chronaxie(duration_data, voltage_amp_data):
    """
    Calculates Rheobase and Chronaxie. The pulse duration and voltage
    amplitude data should be interpolated data to get the most accurate
    results.
    ...
    Returns:
        rheobase (float): Rheobase =  the minimal electric current required
                                      to excite a tissue
        chronaxie (float): chronaxie = minimum time required for an electric
                                       current double the strength of the
                                       rheobase to stimulate a tissue
    """
    # Calculate Rheobase
